...
# 加载保存的最优模型
from model import FusionModel2D3D  # 假设2D+3D模型类定义在 model.py 文件中
...
# 加载测试数据
test_loader_2d = DataLoader(val_graphs, batch_size=8, shuffle=False)
test_loader_3d = TorchDataLoader(test_dataset, batch_size=8, shuffle=False)

# 合并测试数据加载器
class CombinedTestLoader2D3D:
...
test_loader = CombinedTestLoader2D3D(test_loader_2d, test_loader_3d)
...
with torch.no_grad():
    for (data_2d, data_3d) in test_loader:
        ...
        x_3d = x_3d.to(device)
        batch_2d = batch_2d[1].to(device)
        y_3d = y_3d.to(device)

        # 模型预测
        outputs = model(x_2d_nodes, edge_index, x_3d, batch_2d)
